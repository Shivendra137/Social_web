...
export default function App() {
  ...
  const handleLogin = (userType: "citizen" | "municipal", userInfo: { username: string; name: string }) => {
    ...
    if (userType === "municipal") {
      ...
      // Citizens need to select municipality first
      if (!selectedMunicipality) {
        setCurrentPage("municipality-selection");
      } else {
        setCurrentPage("home");
      }
    }
  ...
  const handleMunicipalitySelection = (municipality: Municipality) => {
  ...
  const renderPage = () => {
    switch (currentPage) {
      case "municipality-selection":
        return (
          <MunicipalitySelection 
            onSelectMunicipality={handleMunicipalitySelection}
          />
        );
      case "home":
        return (
      ...
      case "municipality-info":
        // Only allow citizens to view municipality info
        if (currentUser?.type !== "citizen" || !selectedMunicipality) {
          return (
            <div className="min-h-screen bg-background flex items-center justify-center">
              <p className="text-muted-foreground">Access denied. This page is only for citizens with selected municipality.</p>
            </div>
          );
        }
        return (
    ...
    <LanguageProvider>
      <div className="min-h-screen relative overflow-hidden">
        ...
        <div className="relative z-10 min-h-screen bg-gradient-to-br from-orange-50/80 via-white/90 to-green-50/80 backdrop-blur-sm">
          {currentPage !== "municipality-selection" && (
            <Navigation 
